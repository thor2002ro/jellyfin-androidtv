...
internal fun UserPreferences.mpvAudioPolicy(supportedPassthroughMimes: Set<String>): LibMPVAudioPolicy {
	...
	val passthroughCodecs = buildList {
		if (this@mpvAudioPolicy[UserPreferences.ac3Enabled] && MimeTypes.AUDIO_AC3 in supportedPassthroughMimes) add("ac3")
		if (
			this@mpvAudioPolicy[UserPreferences.eac3Enabled] &&
			supportedPassthroughMimes.any { mime -> mime == MimeTypes.AUDIO_E_AC3 || mime == MimeTypes.AUDIO_E_AC3_JOC }
		) add("eac3")
		if (this@mpvAudioPolicy[UserPreferences.dtsEnabled]) {
			if (
		...
		if (this@mpvAudioPolicy[UserPreferences.truehdEnabled] && MimeTypes.AUDIO_TRUEHD in supportedPassthroughMimes) add("truehd")
	}
