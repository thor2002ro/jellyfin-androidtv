	...
	private fun VerticalBrowseGrid(holder: ViewHolder) {
		...
		BoxWithConstraints {
			...
			val viewportHeight = calculateBrowseGridViewportHeight(
				...
				cardHeight = configValue.cardHeight.toFloat(),
				verticalSpacing = configValue.verticalSpacing.toFloat(),
...
internal fun calculateBrowseGridViewportHeight(
	...
	if (maxHeight <= 0f || cardHeight <= 0f) return maxHeight.coerceAtLeast(0f)

	val totalPadding = verticalPadding.coerceAtLeast(0f) * 2f
	val availableHeight = (maxHeight - totalPadding).coerceAtLeast(cardHeight)
	val rowExtent = cardHeight + verticalSpacing
	val visibleRows = floor((availableHeight + verticalSpacing) / rowExtent).toInt().coerceAtLeast(1)
	return (totalPadding + cardHeight * visibleRows + verticalSpacing * (visibleRows - 1))
