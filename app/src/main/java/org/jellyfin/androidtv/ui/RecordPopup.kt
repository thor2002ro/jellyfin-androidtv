...
class RecordPopup(
	...
	fun setContent(
		...
		setTimelineRow(context, timeline, program)
	...
	private fun setTimelineRow(context: Context, timelineRow: LinearLayout, program: BaseItemDto) {
		timelineRow.removeAllViews()
		val local = program.startDate ?: return

		timelineRow.addView(TextView(mContext).apply {
			text = mContext.getString(R.string.lbl_on)
		})
		timelineRow.addView(TextView(mContext).apply {
			text = program.channelName
			setTypeface(null, Typeface.BOLD)
			setTextColor(mContext.resources.getColor(android.R.color.holo_blue_light))
		})
		timelineRow.addView(TextView(mContext).apply {
			text = StringBuilder()
				.append(TimeUtils.getFriendlyDate(context, local))
				.append(" @ ")
				.append(mContext.getTimeFormatter().format(local))
				.append(" (")
				.append(
