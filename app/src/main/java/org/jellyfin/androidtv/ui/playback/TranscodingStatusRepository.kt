...
object TranscodingStatusFormatter {
	...
	fun compact(info: TranscodingInfo?): String? {
		...
		return buildString {
			...
			appendInline(info.transcodeReasons.takeIf { it.isNotEmpty() }?.joinToString(", ") { it.displayName() }?.let { "Reason: $it" })
	...
	fun status(info: TranscodingInfo?): String? {
		...
		return buildString {
			appendSpacedInline(progress(info))
			appendSpacedInline(speed(info))
			appendSpacedInline(info.hardwareAccelerationType?.takeUnless { it == HardwareAccelerationType.NONE }?.toString())
		}.takeIf { it.isNotBlank() }
	}
	...
	fun videoConversion(
		...
		return buildString {
			...
			appendInline(videoDetails(info))
		}.takeIf { it.isNotBlank() }
	}
	...
	fun audioConversion(
		...
		return buildString {
			...
			info.audioChannels?.takeIf { it > 0 }?.let { appendInline("${it}ch") }
		}.takeIf { it.isNotBlank() }
	}
	...
	private fun Int.formatBitrate(): String = when {
	...
	private fun StringBuilder.appendSpacedInline(value: String?) {
		if (value.isNullOrBlank()) return
		if (isNotEmpty()) append(' ')
		append(value)
	}
