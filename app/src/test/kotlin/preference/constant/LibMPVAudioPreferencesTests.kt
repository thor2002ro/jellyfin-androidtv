...
class LibMPVAudioPreferencesTests : FunSpec({
	test("upstream AC3 mode controls MPV without bypassing route support") {
		for (mode in BitstreamAudioMode.entries) {
			val preferences = preferences()
			every { preferences[UserPreferences.bitstreamAc3] } returns mode
			preferences.mpvAudioPolicy(emptySet()).audioSpdif shouldBe ""
			preferences.mpvAudioPolicy(setOf(MimeTypes.AUDIO_AC3)).audioSpdif shouldBe
				if (mode == BitstreamAudioMode.DISABLE) "" else "ac3"
		}
	}

	test("AC3 disable blocks MPV EAC3 and JOC even with an explicit EAC3 enable") {
		val preferences = preferences(ac3 = false)
		every { preferences[UserPreferences.bitstreamEac3] } returns BitstreamAudioMode.ENABLE
		preferences.mpvAudioPolicy(setOf(MimeTypes.AUDIO_E_AC3, MimeTypes.AUDIO_E_AC3_JOC)).audioSpdif shouldBe ""
	}

	test("all codec modes retain MPV variant support and hardware checks") {
		val formats = mapOf(
			BitstreamAudioFormat.AC3 to "ac3",
			BitstreamAudioFormat.EAC3 to "eac3",
			BitstreamAudioFormat.DTS to "dts,dts-hd",
			BitstreamAudioFormat.TRUEHD to "truehd",
		)
		val supportedMimes = setOf(MimeTypes.AUDIO_AC3, MimeTypes.AUDIO_E_AC3_JOC, MimeTypes.AUDIO_DTS_EXPRESS, MimeTypes.AUDIO_DTS_HD, MimeTypes.AUDIO_TRUEHD)
		for ((format, codecs) in formats) for (mode in BitstreamAudioMode.entries) {
			val preferences = preferences(eac3 = false, dts = false, truehd = false)
			every { preferences[format.preference] } returns mode
			val route = when (format) {
				BitstreamAudioFormat.AC3 -> setOf(MimeTypes.AUDIO_AC3)
				else -> supportedMimes - MimeTypes.AUDIO_AC3
			}
			preferences.mpvAudioPolicy(route).audioSpdif shouldBe if (mode == BitstreamAudioMode.DISABLE) "" else codecs
			preferences.mpvAudioPolicy(emptySet()).audioSpdif shouldBe ""
		}
	}
