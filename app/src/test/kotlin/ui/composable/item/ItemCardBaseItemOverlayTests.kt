...
class ItemCardBaseItemOverlayTests : FunSpec({
	...
	test("stored progress is limited to videos and books") {
		listOf(MediaType.VIDEO, MediaType.BOOK, MediaType.AUDIO, MediaType.PHOTO).forEach { mediaType ->
			val item = BaseItemDto(
				id = UUID.randomUUID(),
				type = BaseItemKind.MOVIE,
				mediaType = mediaType,
			...
			item.storedPlayedProgress() shouldBe if (mediaType == MediaType.VIDEO || mediaType == MediaType.BOOK) 0.5f else null
		}
	}
