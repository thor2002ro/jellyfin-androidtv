...
class AudioPassthroughSupportTests : FunSpec({
	...
	test("fully supported DTS family lists every variant in display order") {
		...
		supportedPassthroughAudioFormats(AudioPassthroughFamily.DTS, supportedMimes)
	...
	test("EAC3 JOC support remains distinct from base EAC3") {
		...
		supportedPassthroughAudioFormats(AudioPassthroughFamily.EAC3, supportedMimes)
			.map { format -> format.label } shouldBe listOf("E-AC3 JOC")
	}

	test("unsupported family has no supported formats") {
		supportedPassthroughAudioFormats(AudioPassthroughFamily.TRUEHD, emptySet()).shouldBeEmpty()
	}
})
