...
class AppUpdaterTest : FunSpec({
	...
	test("clears the updater directory when no download is retained") {
		val directory = createTempDirectory().toFile()
		directory.resolve("stale.apk").writeText("stale")
		directory.resolve("note.txt").writeText("stale")
		directory.resolve("nested").mkdirs()
		directory.resolve("nested/stale.apk.part").writeText("stale")
